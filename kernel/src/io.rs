...
use crate::bindings::imports::{M_DEVBUF, M_WAITOK, free, malloc, uprintf};
use core::{fmt, ptr};
...
impl fmt::Write for KernelDebugWriter {
    fn write_str(&mut self, message: &str) -> fmt::Result {
        unsafe {
            let ptr = malloc(message.len() + 1, &mut M_DEVBUF[0], M_WAITOK) as *mut c_char;
            if ptr.is_null() {
                ...
                uprintf(ptr);
                free(ptr as *mut c_void, &mut M_DEVBUF[0]);
            }
        }
        Ok(())
    }
